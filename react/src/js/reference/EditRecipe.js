...
class EditRecipe extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      activeColor: "info",
      error_message: '',
      // hardcoded since we only have one actuator in the PFC.
      output_peripherals_for_select: [{
        "name": "LED",
        "units": "cm, umol/m2/s, nm: %",
        "recipe_variables": "light_illumination_distance_cm, light_ppfd_umol_m2_s, light_spectrum_nm_percent"
      }],
      env_outputs_for_select: [],
      env_output_index_for_select: -1,
      envs_tr: [],
      phases_tr: [],
    ...
    for (var index in this.state.output_peripherals_for_select) {
      var p = this.state.output_peripherals_for_select[index];
      this.state.env_outputs_for_select.push({
        'label': p.name,
        'value': p
      });
    }
  ...
  handleChangeAuthors(event) {
    ...
    for (var i = 0; i < authors_arr.length; i++) {
      recipe_obj.authors.push({
        'name': authors_arr[i].trim(),
        'email': null,
        'uuid': null
      });
    }
  ...
  handleChangeCultivars(event) {
    ...
    for (var i = 0; i < cults_arr.length; i++) {
      recipe_obj.cultivars.push({
        'name': cults_arr[i].trim(),
        'uuid': null
      });
    }
    this.setState({ recipe_obj: recipe_obj });
  ...
  handleChangeCultivationMethods(event) {
    ...
    for (var i = 0; i < meth_arr.length; i++) {
      recipe_obj.cultivation_methods.push({
        'name': meth_arr[i].trim(),
        'uuid': null
      });
    }
    this.setState({ recipe_obj: recipe_obj });
  ...
  buildEnvRowsFromState = () => {
    ...
    for (var env_id in envs) {
        ...
        for (j = 0; j < vars.length; j++) {
          ...
          if (variable !== null && typeof variable === 'object') {
            ...
            for (var multi_value_var_name in variable) {
              ...
              input.push(<input className="input-row" type="number" min="0"
                key={key}
                id={key}
              ...
              multi_table.push(<tr key={this.getNextEnvKey()}>
                ...
                <td className="value-cell">{input}</td>
                <td>{units[j]}</td>
              </tr>);
            }
      ...
      envs_tr.push(<tr key={this.getNextEnvKey()}>
        ...
        <td>{env_id}</td>
        <td><input className="input-row" type="text"
          id={this.getNextEnvKey()}
          placeholder="Environment name"
          value={env.name}
          onChange={this.handleChangeEnvName.bind(this, env_id)} /></td>
        <td className="select-cell"><Select isClearable isSearchable
          defaultValue={this.state.env_outputs_for_select[
            this.state.env_output_index_for_select]}
          id={env_id}
          name={env_id}
          onChange={this.handleEnvOutputChange.bind(this, env_id)}
          options={this.state.env_outputs_for_select} /></td>
        <td>
          {periph_td}
        </td>
      </tr>);
    ...
    for (var phase_index in phases) {
      ...
      for (var cycle_index in phase.cycles) {
        ...
        cycles_tr.push(<tr key={this.getNextPhaseKey()}>
          ...
          <td><input className="input-row" type="number" min="0"
            id={this.getNextPhaseKey()}
            placeholder="Hours"
          ...
          <td className="env-cell"><Select isClearable isSearchable
      ...
      phases_tr.push(<tr key={this.getNextPhaseKey()}>
        ...
        <td><input className="input-row" type="number" min="1"
          id={this.getNextPhaseKey()}
          placeholder="Repeat"
          value={phase.repeat}
  ...
  handleEnvOutputChange = (env_id, event) => {
    ...
    if (event === undefined || event === null) {
      ...
      this.setState({ env_output_index_for_select: -1 });
      this.setState({ environments: envs }, function () {
        this.buildEnvRowsFromState();
      });
      return;
    ...
    this.setState({ env_output_index_for_select: 0 });

    // Add the variables from the peripheral to the environment.
    let selected_periph = event.value;
        ...
        <Form className={"m-2"}>
          ...
          <div className="row">
            ...
            <div className="col-md-11">
              ...
              <Button className="submit-button-row" color="primary"
